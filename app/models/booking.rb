class Booking < ApplicationRecord
  ...
  class << self
    ...
    #   Booking.create_with_add_ons(user: user, tour: tour)
    #
    # @example Create a booking with add-ons
    #   Booking.create_with_add_ons(user: user, tour: tour, add_on_ids: [1, 2, 3])
    #
    # @example Create a booking with add-ons and a note
    #   Booking.create_with_add_ons(user: user, tour: tour, add_on_ids: [1], note: "Special requests")
    #
    ...
    def create_with_add_ons(user:, tour:, add_on_ids: [], note: nil)
